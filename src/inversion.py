...
def reconstruct(
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=[num_iterations // 2.667, num_iterations // 1.6, num_iterations // 1.142], gamma=0.1
    )
    ...
    vutils.save_image(
        dummy_input.detach().cpu(), f"{output_path}/batch_{batch_idx}/reconstruction.png", nrow=8, normalize=True
    )
    if wandb_log:
